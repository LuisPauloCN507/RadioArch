...
const RadioInfo = ({ radio }) => (
  <div className="flex items-center gap-4 w-1/3 min-w-0">
    ...
    <img 
      src={radio.logo} 
      alt={radio.name}
      className="w-12 h-12 rounded border border-zinc-700 hidden sm:block" 
    />
    <div className="truncate">
      <h4 className="font-bold text-sm text-white truncate">{radio.name}</h4>
...
const VolumeControl = ({ volume, onVolumeChange }) => (
  <div className="flex items-center gap-3 w-1/3 justify-end group">
    {volume === 0 ? (
      ...
      <Volume2 size={20} className="text-zinc-400 group-hover:text-orange-500" />
    )}
    <input 
      type="range" 
      min="0" 
